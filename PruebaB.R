...
mapa <- ne_states(
  ...
  mutate(
    ...
    ENT_OCURR = stri_trans_general(ENT_OCURR, "Latin-ASCII"),
    ENT_OCURR = recode(
      ENT_OCURR,
...
df_ocu_sep <- df_ocu %>%
  mutate(
    OCUPACION = recode(as.character(OCUPACION),
                       ...
                       "051" = "Trabajadores en la preparación y servicio de alimentos y bebidas y de esparcimiento y hoteleríaM"
    )
  ) %>%
  arrange(SEXO, desc(homicidios)) %>%
  group_by(SEXO) %>%
